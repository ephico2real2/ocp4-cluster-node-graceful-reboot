...
SPINNER_PID=""
ENABLE_SPINNER=true
NODE_TYPE=""  # Track the current node type globally
START_TIME=$(date +%s)
...
wait_for_node_ready() {
    local node=$1
    local dry_run=$2
    # For infra nodes, increase the attempts
    if [[ "$node" =~ ^infra || "$NODE_TYPE" == "infra" ]]; then
        max_attempts=$((NODE_READY_TIMEOUT * 2))
        log "$LOG_LEVEL_INFO" "Using extended timeout for infrastructure node ($max_attempts attempts)"
    fi
    ...
    if [ "$dry_run" = true ]; then
        ...
        # Check node status with timeout and better error handling
        local node_status
        node_status=$(timeout $OC_COMMAND_TIMEOUT oc get node "$node" -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}' 2>&1)
        local exit_code=$?
        
        if [ $exit_code -ne 0 ]; then
            log "$LOG_LEVEL_WARNING" "Command failed with exit code $exit_code: $node_status"
        else 
            if [[ "$node_status" == "True" ]]; then
                ...
                log "$LOG_LEVEL_DEBUG" "Node status is '$node_status', waiting for 'True'"
            fi
        fi
...
check_node_debug_access() {
        ...
        local debug_output
        debug_output=$(timeout $OC_COMMAND_TIMEOUT oc debug node/"$node" -- chroot /host ls / 2>&1)
        local exit_code=$?
        ...
        if [ $exit_code -eq 0 ]; then
            ...
            log "$LOG_LEVEL_WARNING" "Debug access failed with exit code $exit_code: ${debug_output:0:100}..."
        fi
        return 0
    fi
...
process_nodes_parallel() {
    ...
    while [ $i -lt $total_nodes ]; do
        ...
        if [ "$dry_run" = false ]; then
            ...
            for j in $(seq $batch_start $batch_end); do
                ...
                log "$LOG_LEVEL_INFO" "Beginning post-reboot verification for node $node..."
        ...
        if [ "$dry_run" = false ]; then
            ...
            for j in $(seq $batch_start $batch_end); do
                ...
                local was_skipped=false
                    ...
                    log "$LOG_LEVEL_WARNING" "Node $node did not become Ready within the timeout period. Run with --verbose for more details."
                local was_skipped=false
                ...
                if $USE_COLORS; then
                    ...
                    log "$LOG_LEVEL_WARNING" "Node $node is not accessible via debug within the timeout period. Run with --verbose for more details."
    ...
    # Add helpful information if nodes failed
    if [[ $failed_nodes -gt 0 ]]; then
        log "$LOG_LEVEL_WARNING" "Some nodes failed during the reboot process."
        log "$LOG_LEVEL_WARNING" "This could be because:"
        log "$LOG_LEVEL_WARNING" "  1. The node did not become Ready within the timeout period"
        log "$LOG_LEVEL_WARNING" "  2. The node was not accessible via debug connection"
        log "$LOG_LEVEL_WARNING" "  3. The node could not be uncordoned"
        log "$LOG_LEVEL_WARNING" ""
        log "$LOG_LEVEL_WARNING" "For certain node types, this sometimes happens because:"
        log "$LOG_LEVEL_WARNING" "  - They host critical components with strict PodDisruptionBudgets"
        log "$LOG_LEVEL_WARNING" "  - They take longer than the default timeout to become fully ready"
        log "$LOG_LEVEL_WARNING" "  - The debug connection might be blocked by security policies"
        log "$LOG_LEVEL_WARNING" ""
        log "$LOG_LEVEL_WARNING" "Consider using longer timeouts with the --timeout option"
        log "$LOG_LEVEL_WARNING" "Or check the node status manually: oc get node <node-name>"
    fi
...
main() {
    ...
    if [[ $custom_timeout -gt 0 ]]; then
        ...
        NODE_DEBUG_TIMEOUT=$((NODE_READY_TIMEOUT / 2))
        log "$LOG_LEVEL_INFO" "Using custom timeout: $custom_timeout seconds ($NODE_READY_TIMEOUT attempts for ready, $NODE_DEBUG_TIMEOUT for debug)"
    fi
    
    # Adjust timeouts for certain node types
    if [[ "$NODE_TYPE" == "infra" || "$NODE_TYPE" == "master" ]]; then
        # For infra and master nodes, default to a longer timeout if not explicitly set
        if [[ $custom_timeout -eq 0 ]]; then
            NODE_READY_TIMEOUT=$((NODE_READY_TIMEOUT * 2))
            NODE_DEBUG_TIMEOUT=$((NODE_DEBUG_TIMEOUT * 2))
            log "$LOG_LEVEL_INFO" "Using extended timeouts for $NODE_TYPE nodes: $NODE_READY_TIMEOUT attempts for ready, $NODE_DEBUG_TIMEOUT for debug"
        fi
    fi
    if $USE_COLORS; then
    ...
    if [[ -n "$node_type" ]]; then
        NODE_TYPE="$node_type"  # Save the node type globally
        if [ "$dry_run" = true ]; then
